document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("startExam").addEventListener("click", async () => {
        try {
            ...
            cards = (cards || []).map(c => ({ ...c, CardID: Number(c.CardID) }))
            cards = cards.filter(c => !knownCards.includes(c.CardID))
            if (cards.length > 10) cards = cards.sort(() => 0.5 - Math.random()).slice(0, 10)
            ...
            button.addEventListener("click", async () => {
                ...
                await fetch(`${baseURL}/api/sets/${setId}/cards`, {
                    ...
                    body: JSON.stringify(learnedNow)
                })
            })
        } catch (err) {
            console.error(err)
